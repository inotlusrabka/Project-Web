    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            motherboards.forEach(function(mb) {
                ...
                $('#motherboard').append('<option value="' + mb.item_id + '" data-socket="' + mb.socket + '" data-price="' + mb.price + '" data-img="' + mb.image_url + '">' + optionText + '</option>');
            });

            // Populate GPU dropdown
            gpus.forEach(function(gpu) {
                var optionText = gpu.brand + " " + gpu.item_name + " | Rp" + gpu.price.toLocaleString('id-ID');
                $('#gpu').append('<option value="' + gpu.item_id + '" data-price="' + gpu.price + '" data-img="' + gpu.image_url + '">' + optionText + '</option>');
            });

            // Populate RAM dropdown
            rams.forEach(function(ram) {
                var optionText = ram.brand + " " + ram.item_name + " | Rp" + ram.price.toLocaleString('id-ID');
                $('#ram').append('<option value="' + ram.item_id + '" data-price="' + ram.price + '" data-img="' + ram.image_url + '">' + optionText + '</option>');
            });
                ...
                if (selectedSocket) {
                    ...
                    processors.forEach(function(cpu) {
                        if (cpu.socket === selectedSocket) {
                            ...
                            $('#cpu').append('<option value="' + cpu.item_id + '" data-price="' + cpu.price + '" data-img="' + cpu.image_url + '">' + optionText + '</option>');
                        }
                    });
                } else {
            ...
            powerSupplies.forEach(function(ps) {
                ...
                $('#psu').append('<option value="' + ps.item_id + '" data-price="' + ps.price + '" data-img="' + ps.image_url + '">' + optionText + '</option>');
            });
